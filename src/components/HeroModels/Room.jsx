...
//   const base = import.meta.env.BASE_URL; // will be '/' in dev and '/ankit_portfolio/' in production
...
export function Room(props) {
  const base = import.meta.env.BASE_URL; // ✅ must be defined before use
  const screensRef = useRef();

  // ✅ Correct asset paths using Vite base
  const { nodes, materials } = useGLTF(`${base}models/optimized-room.glb`);
  const matcapTexture = useTexture(`${base}images/textures/mat1.png`);
...
// ✅ preload moved inside a function scope so `base` exists
